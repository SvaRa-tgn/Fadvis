let requestData = {
    auth: function (){
        $('.js-auth').submit(function (e){
            ...
            notif.removeClass('form-notification');
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    if (form.attr('data-auth') === 'reset') {
                        notify.text(response.message);
                        close.removeClass('hide');
                        linkAdmin.addClass('hide');
                    } else {
                        notify.text('Вы успешно авторизовались');
                        close.addClass('hide');
                        linkAdmin.removeClass('hide');
                        linkAdmin.attr('href', response.route);
                ...
                error: (response) => {
                    ...
                    if (response.status === 422) {
                        if (form.attr('data-auth') === 'reset') {
                            notif.addClass('form-notification');
                            notif.attr('data-notification', response.responseJSON.text);
                            close.removeClass('hide');
                            linkAdmin.addClass('hide');
                        } else {
